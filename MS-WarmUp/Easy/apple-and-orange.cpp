...
void countApplesAndOranges(
    int const s, // house_left
    int const t, // house_right
    int const a, // apple_tree
    int const b, // orange_tree
    vector<int> const& apples,
    vector<int> const& oranges) noexcept
{
    cout << count_if(apples.cbegin(), apples.cend(), [a, s, t](int const d) {
        return a+d >= s && a+d <= t;
    }) << '\n' <<
    count_if(oranges.cbegin(), oranges.cend(), [b, s, t](int const d) {
        return b+d >= s && b+d <= t;
    }) << '\n';
}
